...
\begin{itemize}
\item
  \href{https://www.juliasoft.com/solutions/}{Julia analyzer},
\item
...
\section{Nullness\_Lite: An Unsound Option of the Nullness Checker for fewer false positives\label{nullness-lite}}

\subsection{Introduction\label{nullness-lite-introduction}}

Nullness\_Lite is a lite option of the Nullness Checker in the Checker Framework
to detect nullness bugs in java source files. It disables the following features
of Nullness Checker to obtain more desireable traits. To be specific, the Nullness
Checker with Nullness\_Lite option enabled will be faster and easier to use by
delibrately giving up soundness.

\begin{tabular}{|l|l|}
\hline
\textbf{Featrue Disabled} & \textbf{Substitute Behavior of the Nullness\_Lite option}\\ \hline
Initialization Checker & Assume all values (fields \& class variables) initialized\\ \hline
Map Key Checker & Assume all keys are in the map and \code{Map.get(key)} returns \code{@NonNull}\\ \hline
Invalidation of dataflow & Assume no aliasing and all methods are \code{@SideEffectFree} \\ \hline
Boxing of primitives & Assume the boxing of primitives return the same object and \code{BoxedClass.valueOf()} are \code{@Pure}\\ \hline
\end{tabular}


\subsection{Who wants to use Nullness\_Lite option?\label{nullness-lite-who}}

Java developers who would like to get a trustable analysis on their source files,
but hesitate to spend time running full verification tool such as Nullness Checker.
They can run the Nullenss\_Lite option instead to get a fast glimpse on their files
before they upgrade to the Nullness Checker.


\subsection{How to use the Nullness\_Lite?\label{nullness-lite-how}}

Please follow the instructions for installation first. To compile the Nullness Checker,
please follow the instructions in Section \href{https://checkerframework.org/manual/#running}
{2.2} for information about running a checker.

Users run Nullness\_Lite by passing an extra argument when running Nullness Checker
...
* Notice that the behavior is undefined if \code{-ANullnessLite option} is passed to a
different checker.

\subsection{More information\label{nullness-lite-more}}

\begin{itemize}
\item
See the \href{https://github.com/979216944/checker-framework}{implementation}
\item
See the \href{https://github.com/NullnessLiteGroup/junit4}{evaluation on JUnit4}
\end{itemize}

Refer to the \href{https://github.com/weifanjiang/Nullness_Lite}{complete Nullness\_Lite
user manual on GitHub} for more details.
...
% ../../checker/src/org/checkerframework/checker/nullness/NullnessAnnotatedTypeFactory.java
\begin{figure}
\begin{center}
...
the value is prosent.  This is still easy to forget, however.
...
\href{http://homes.cs.washington.edu/~mernst/advice/nothing-is-better-than-optional.html}{``Nothing
  is better than Java's \<Optional> class''}.)
...
\subsubsection{``error:  the constructor does not initialize fields: \ldots''\label{initialization-warnings-constructor}}
...
\subsubsection{``call to \ldots\ is not allowed on the given receiver''\label{initialization-warnings-receiver}}
...
% Declaration of m() needs:
%   @UnderInitialization(MyClass.class) MyClass this
